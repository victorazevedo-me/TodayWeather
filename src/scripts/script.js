		...
		img.onload = () => {
			...
			img.addEventListener('load', function() {
				...
				colors.forEach(color => {
					...
					if (rgbToLightness(color[0], color[1], color[2]) > brightest) {
						brightest = rgbToLightness(color[0], color[1], color[2]);

						let finalColor = rgbToHsl(color[0], color[1], color[2]);

						finalColor[1] = finalColor[1] + 0.3;
						finalColor[2] = finalColor[2] + 0.1;

						finalColor = hslToRgb(finalColor[0], finalColor[1], finalColor[2])

						console.log(finalColor);

						document.body.style.color = 'rgb(' + finalColor[0] + ',' + finalColor[1] + ',' + finalColor[2] + ')';
